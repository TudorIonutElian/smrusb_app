                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr v-for="(c, index) in cass_list">
                                    ...
                                    <td style="color: green; font-weight: bold;">Achitat</td>
                                </tr>
                                <tr class="tr-sumar" v-if="suma_totala_achitat > 0">
                                    <td colspan="4" class="td-colspan">Valoarea totala a CASS achitat ::</td>
                                    <td colspan="4" class="td-colspan">{{ suma_totala_achitat }} lei </td>
                                </tr>
                                <tr v-if="cass_list.length === 0">
                                    <td colspan="8" class="bg-warning text-center">Nu exista CASS Achitat pentru perioada de referinta.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
export default {
    ...
    methods:{
        calculSumar(cass_list){
            this.suma_totala_achitat = 0;
            cass_list.forEach(cass =>{
                this.suma_totala_achitat += cass.cass_suma;
            })

        },
        async preluareCassNeachitat(){
            this.loading = true;
            await axios.get(`/api/salarizare/cass/achitat/${this.filtrare.institutie}/${this.filtrare.dela}/${this.filtrare.panala}`, {
                headers:{
                    ...
                    this.calculSumar(this.cass_list);
                    this.loading = false;
                }
            )
        },
