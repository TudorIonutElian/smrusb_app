                ...
                <div class="row">
                    <div class="col-12 d-flex mb-2">
                        <div class="d-flex mb-3">
                            ...
                            <div class="d-flex-inner m-2">
                                <label for="institutie">Selectati Anul</label>
                                <select
                                    id="luna"
                                    class="form-control form-select m-2"
                                    aria-label="Default select example"
                                    v-model="data.anulSelectat"
                                    :disabled="data.institutie == null"
                                >
                                    <option v-for="an in this.data.aniCalculati" :value="an">{{ an }}</option>
                                </select>
                            </div>
                            <div class="d-flex-inner m-2">
                                ...
                                <button
                                    ...
                                    :disabled="data.anulSelectat == null"
...
export default {
    ...
    methods:{
        // setareAni
        setareAni(){
            for(let i = this.data.anulCurent; i > (this.data.anulCurent-10); i--){
                this.data.aniCalculati.push(i-1)
            }
        },
        // preluare user acces
        async preluareAcces(){
        ...
        async preluareSalarii(){
            this.data.sumar = 0;
            this.loading = true;
            await axios.get(`/api/salarizare/${this.data.institutie}/anual/${this.data.anulSelectat}`, {
                headers:{
                    ContentType: 'application/json',
                    Authorization : 'Bearer ' + this.data.token
                }
